...
const generateMockGameData = (mode, bestOf) => {
  return { seed: Math.random(), rounds: bestOf, timestamp: Date.now() };
};

export default function Matchmaking({ session, gameMode, onBack, onStartMatch, isDarkMode, language }) {
  const [bestOf, setBestOf] = useState(1);
  const [matchType, setMatchType] = useState(null); // 'public' | 'friend'
  const [loading, setLoading] = useState(false);
  const [friends, setFriends] = useState([]);
  const [matchState, setMatchState] = useState(null); // The match object once created/joined
      ...
      <SafeAreaProvider>
        <SafeAreaView style={[styles.container, isDarkMode ? styles.containerDark : styles.containerLight]}>
          ...
          <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
            ...
            <Text style={[styles.waitingText, isDarkMode ? {color: '#f8fafc'} : {color: '#1e293b'}]}>
              {matchState.is_public ? (language === 'fr' ? "Recherche d'un adversaire..." : "Searching for an opponent...") : (language === 'fr' ? "En attente de l'ami..." : "Waiting for friend...")}
            </Text>
  ...
  const renderFriend = ({ item }) => {
    ...
    return (
      <View style={[styles.friendCard, isDarkMode ? styles.cardDark : styles.cardLight]}>
        <Text style={[styles.friendName, isDarkMode ? {color: '#f8fafc'} : {color: '#1e293b'}]}>{friendData.username}</Text>
      ...
      <SafeAreaView style={[styles.container, isDarkMode ? styles.containerDark : styles.containerLight]}>
        ...
        <View style={[styles.header, isDarkMode ? styles.headerDark : styles.headerLight]}>
          ...
          <Text style={[styles.title, isDarkMode ? styles.titleDark : styles.titleLight]}>
            ...
            {gameMode === 'classic' ? (language === 'fr' ? 'Classique' : 'Classic') : gameMode === 'streak' ? 'Streak' : 'Versus'}
          </Text>
          <View style={{ width: 44 }} />
        </View>
...
const styles = StyleSheet.create({
  ...
  cancelBtn: { marginTop: 30, alignSelf: 'center', padding: 16, backgroundColor: '#ef4444', borderRadius: 12, width: 200 },
